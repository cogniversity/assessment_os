...
export default function LoginPage() {
  ...
  const oidcLogin = async () => {
    try {
      ...
      else setError("OIDC not configured. Use dev login below.");
    } catch (e) {
      setError(e instanceof Error ? e.message : "Login failed");
    }
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-lg ring-1 ring-slate-200 overflow-hidden">
          ...
          <button
            type="button"
            onClick={oidcLogin}
            className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-4 text-sm transition-colors"
          >
            <Shield size={16} />
